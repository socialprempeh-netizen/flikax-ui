    ...
    <ul className="space-y-1">
      {toc.map((item) => (
        <li key={item.id}>
          <a
            href={`#${item.id}`}
            className="block px-3 py-1.5 text-sm text-neutral-600 hover:bg-brand-light hover:text-brand-dark"
          >
            {item.title}
          </a>
        </li>
...
export function TocLayout({ toc, children }: { toc: TocItem[]; children: React.ReactNode }) {
  return (
    <main className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-10 px-4 py-10 sm:flex-row sm:px-6">
      <details className="mb-6 h-fit bg-white p-4 shadow-md sm:hidden">
