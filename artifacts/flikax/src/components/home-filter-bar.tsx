 ...
 * hold) and Sort. Both open as small anchored dropdowns on sm+ and as
 * bottom-anchored panels on mobile. */
export function HomeFilterBar({ filters }: { filters: ListingFilters }) {
  ...
  return (
    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
      ...
      {filtersOpen && (
        <>
          <div className="fixed inset-0 z-40 hidden sm:block" onClick={() => setFiltersOpen(false)} aria-hidden="true" />
          <div
            style={
              anchor
                ? ({ "--fp-top": `${anchor.top}px`, "--fp-left": `${anchor.left}px` } as React.CSSProperties)
                : undefined
            }
            className="fixed inset-x-4 bottom-4 top-auto z-50 max-h-[80vh] overflow-y-auto rounded-2xl border border-neutral-200 bg-white shadow-xl sm:inset-x-auto sm:bottom-auto sm:left-[var(--fp-left)] sm:top-[var(--fp-top)] sm:w-80"
          >
